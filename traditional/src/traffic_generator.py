"""
ESPMotion - WiFi Traffic Generator
...
class TrafficGenerator:
    ...
    def _create_socket(self):
        """Create a non-blocking UDP socket for DNS queries."""
    ...
    def _dns_task(self):
        ...
        # Count consecutive send failures so we can recreate a wedged
        # socket in-thread (avoids spawning a new thread, which hits
        # MicroPython's hard thread-count limit on ESP32).
        ...
        while self.running:
            try:
                ...
                try:
                    ...
                    # A sustained run of failures means the UDP socket is
                    # wedged — lwIP pressure can leave it unusable even
                    # after memory frees. Recreate it in-thread; this is
                    # what actually revives CSI, and reusing the same
                    # thread avoids MicroPython's "can't create thread"
                    # limit that a stop()/start() cycle would hit.
                    if consecutive_errors >= 40:
                        print(f"Traffic generator: recreating wedged socket "
                        ...
                        try:
                            ...
                            print(f"Traffic generator: socket recreate failed ({ce})")
                        consecutive_errors = 0
                        gc.collect()
                        time.sleep_ms(100)
                        next_send_time = time.ticks_us()
                    ...
                    # On ENOMEM (errno 12), back off so lwIP can free pbufs.
                    # Without this, we hammer sendto() at full rate and lwIP
                    # never recovers, starving CSI reception too.
