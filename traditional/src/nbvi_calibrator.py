...
class NBVICalibrator:
    ...
    def __init__(self, buffer_size=None, mvs_window_size=None,
        ...
        # Batch write buffer to reduce flash I/O overhead. Original size
        # (100 × 64 = 6400 bytes) is too large for the fragmented heap on
        # plain ESP32; 25 × 64 = 1600 bytes still amortizes flash writes
        # ~25x while fitting in a contiguous chunk.
    ...
    def _validate_subcarriers(self, band):
        ...
        # Catch MemoryError from incremental list growth (mv_values.append
        # can fail when the backing array needs to double on a fragmented
        # heap). Return whatever we collected so far — a partial sample
        # still gives a usable adaptive threshold.
        try:
            ...
            print(f"NBVI: validate OOM at pkt {pkt_idx} with {len(mv_values)} samples ({me})")
            gc.collect()
    ...
    def _analyze_candidate_window(self, start_idx, window_size):
        """
        Analyze a single candidate window: compute per-subcarrier metrics and
        build the 4 candidate bands.

        Returns (band_candidates, all_metrics) or None if the window is empty.

        Lives in its own method so a MemoryError unwinds the entire frame
        (raw_data, all_metrics, sorted_*, band_*) — `del locals()[name]`
        does not actually free names in MicroPython.
        """
        self._file.seek(start_idx * NUM_SUBCARRIERS)
        # Up to 200 * 64 = 12800 bytes; with window_size=75 it's 4800 bytes.
        ...
        for sc in range(NUM_SUBCARRIERS):
            ...
            mean = sum(vals) / count
            var = sum((v - mean) * (v - mean) for v in vals) / count
            ...
            # MAD — sort vals in place then reuse the same list for abs devs.
            vals.sort()
            median = vals[count // 2]
        ...
        if len(band_entropy) == BAND_SIZE:
            band_candidates.append(band_entropy)
        if len(band_mad) == BAND_SIZE and band_mad not in band_candidates:
            band_candidates.append(band_mad)
        if len(band_classic_spaced) == BAND_SIZE and band_classic_spaced not in band_candidates:
            band_candidates.append(band_classic_spaced)
        if len(band_classic) == BAND_SIZE and band_classic not in band_candidates:
            band_candidates.append(band_classic)

        gc.collect()
        return band_candidates, all_metrics
    ...
    def calibrate(self, hint_band=None):
        ...
        # Per-candidate analysis is in a helper method so its locals truly go
        # out of scope (and get GC'd) when MemoryError unwinds the frame.
        # `del locals()[name]` does not actually free names in MicroPython.
        analysis_oom = False
        for idx, (start_idx, window_variance) in enumerate(candidates):
            try:
                gc.collect()
                result = self._analyze_candidate_window(start_idx, window_size)
            except MemoryError as me:
                # Function frame already dropped — all locals gone.
                analysis_oom = True
                gc.collect()
                gc.collect()
                print("NBVI: candidate analysis OOM, using default band")
                break

            if result is None:
                continue
            band_candidates, all_metrics = result

            for candidate_band in band_candidates:
                ...
                try:
                    ...
                    analysis_oom = True
                    gc.collect()
                    print("NBVI: validate OOM during candidate eval, using default band")
                    break
