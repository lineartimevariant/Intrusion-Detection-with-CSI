"""
ESPMotion - Main Application

Motion detection using WiFi CSI with MVS algorithm.
Main entry point for the ESPMotion system running on ESP32.
...
def reconnect_wifi(wlan, force=False):
    """
    Restore the WiFi link if it has dropped.

    The link can fall over silently: the traffic generator's sendto()
    keeps succeeding (lwIP just queues the packet) while nothing is
    actually transmitted, so CSI dries up with no error anywhere. It can
    even happen while wlan.isconnected() still reports True — so callers
    that already know CSI has stalled pass force=True to re-associate
    unconditionally. CSI capture is re-armed after the re-association.

    Returns True if the link is up afterwards.
    """
    if wlan.isconnected() and not force:
        return True

    print('[WARN] Re-associating WiFi...')
    try:
        wlan.disconnect()
    except Exception:
        pass
    ...
    # Honour an optional BSSID lock, same as connect_wifi().
    bssid = None
    bssid_hex = getattr(config, 'WIFI_BSSID', None)
    if bssid_hex:
        bssid_clean = bssid_hex.replace(':', '').replace('-', '')
    ...
    try:
        ...
        print(f'[WARN] WiFi connect call failed: {e}')
        return False
    ...
    if not wlan.isconnected():
        print('[WARN] WiFi reconnect timed out')
        return False
    ...
    # CSI capture must be re-armed after a re-association, otherwise the
    # link is up but csi_read() returns nothing.
    try:
        wlan.csi_disable()
    except Exception:
        pass
    try:
        ...
        print(f'[WARN] csi_enable after reconnect failed: {e}')
...
def main():
    ...
    if needs_calibration:
        # Defrag before band calibration — the CSI verification loop above
        # allocates frame objects that fragment the heap.
        for _ in range(4):
            gc.collect()
        run_band_calibration(wlan, detector, traffic_gen, g_state.chip_type)
        # NBVI analysis stops the traffic generator for ~10s; the WiFi
        # link can drop in that silent window. Restore it before the loop.
        reconnect_wifi(wlan)
    else:
    ...
    # CSI starvation watchdog: if the network stack jams (lwIP out of
    # buffers), the traffic generator can't send and CSI dries up — the
    # loop then spins silently, looking like a hang. Track time since the
    # last frame and self-heal by dropping MQTT (frees the lwIP memory its
    # socket holds, letting the traffic generator and CSI recover).
    ...
    try:
        while True:
            ...
            # Publish-only MQTT: the firmware never reads inbound messages.
            # Outbound publishing in publish_state() handles its own timeouts.
            ...
            if frame:
                ...
                # CSI starvation watchdog — checked ~every 1024 idle spins
                # to keep it cheap (~0.1s of idle at 100us/spin).
                idle_check += 1
                if idle_check & 0x3FF == 0:
                    gap_ms = time.ticks_diff(time.ticks_ms(), last_frame_ms)

                    if gap_ms > CSI_REBOOT_MS:
                        # Stage 2: still no CSI after the recovery attempt —
                        # the WiFi/network stack is wedged. A reboot is the
                        # only reliable cure on ESP32; it re-calibrates and
                        # reconnects automatically (~40s).
                        print('[FATAL] No CSI for 20s — network stack wedged, rebooting')
                        try:
                            if mqtt_connected:
                                mqtt_handler.disconnect()
                        except Exception:
